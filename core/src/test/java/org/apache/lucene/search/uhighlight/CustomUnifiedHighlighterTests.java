...
public class CustomUnifiedHighlighterTests extends ESTestCase {
    public void testCustomUnifiedHighlighter() throws Exception {
        ...
        for (UnifiedHighlighter.OffsetSource forceOffsetSource : UnifiedHighlighter.OffsetSource.values()) {
            if (forceOffsetSource == UnifiedHighlighter.OffsetSource.NONE_NEEDED) {
                continue;
            }
            CustomUnifiedHighlighter highlighter = new CustomUnifiedHighlighter(searcher, iwc.getAnalyzer(),
                new CustomPassageFormatter("<b>", "</b>", new DefaultEncoder()), null, fieldValue,
                forceOffsetSource, false, true);
    ...
    public void testNoMatchSize() throws Exception {
        ...
        for (UnifiedHighlighter.OffsetSource forceOffsetSource : UnifiedHighlighter.OffsetSource.values()) {
            ...
            CustomUnifiedHighlighter highlighter = new CustomUnifiedHighlighter(searcher, analyzer, passageFormatter,
                null, firstValue, forceOffsetSource, false, true);
            Snippet[] snippets = highlighter.highlightField("body", query, docId, 5);
            assertThat(snippets.length, equalTo(0));

            highlighter = new CustomUnifiedHighlighter(searcher, analyzer, passageFormatter, null, firstValue,
                forceOffsetSource, true, true);
    ...
    public void testRequireFieldMatch() throws Exception {
        ...
        for (UnifiedHighlighter.OffsetSource forceOffsetSource : UnifiedHighlighter.OffsetSource.values()) {
            ...
            CustomUnifiedHighlighter highlighter = new CustomUnifiedHighlighter(searcher, analyzer,
                passageFormatter, null, firstValue, forceOffsetSource, false, false);
            Snippet[] snippets = highlighter.highlightField("none", query, docId, 5);
            assertThat(snippets.length, equalTo(2));
            assertThat(snippets[0].getText(), equalTo("This is a <b>test</b>."));
            assertThat(snippets[1].getText(), equalTo("Just a <b>test</b> highlighting from unified."));
        }
