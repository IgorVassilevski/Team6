...
public class CustomUnifiedHighlighter extends UnifiedHighlighter {
    ...
    protected BytesRef[] filterExtractedTerms(String field, SortedSet<Term> queryTerms) {
        if (requireFieldMatch == false) {
            SortedSet<BytesRef> terms = new TreeSet<>();
            for (Term term : queryTerms) {
                terms.add(term.bytes());
            }
            return terms.toArray(new BytesRef[terms.size()]);
        }
        return super.filterExtractedTerms(field, queryTerms);
    }

    @Override
    protected PhraseHelper getPhraseHelper(String field, Query query, EnumSet<HighlightFlag> highlightFlags) {
        if (requireFieldMatch == false) {
            super.getPhraseHelper(null, query, highlightFlags);
        }
        return super.getPhraseHelper(field, query, highlightFlags);
    }
}
